...
public class DeliveryRouteOptimization {
    ...
    private static int dynamicProgrammingTSPHelper(int pos, int mask, int[][] dist, int[][] memo, int VISITED_ALL, String[][] paths) {
        if (mask == VISITED_ALL) {
            return dist[pos][0];
        }
        if (memo[pos][mask] != -1) {
        ...
        for (int i = 0; i < dist.length; i++) {
            if ((mask & (1 << i)) == 0) {
                int newCost = dist[pos][i] + dynamicProgrammingTSPHelper(i, mask | (1 << i), dist, memo, VISITED_ALL, paths);
                if (newCost < minCost) {
                    minCost = newCost;
    ...
    public static String backtrackingTSP(int[][] dist) {
        ...
        tspBacktracking(0, dist, visited, n, 1, 0, path, minCost, bestPath);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(">> Greedy TSP Solution:");
        System.out.println(greedyTSP(distanceMatrix) + "\n");
    
        System.out.println(">> Dynamic Programming TSP Solution:");
        System.out.println(dynamicProgrammingTSP(distanceMatrix) + "\n");
    
        System.out.println(">> Backtracking TSP Solution:");
        System.out.println(backtrackingTSP(distanceMatrix) + "\n");
    
        System.out.println(">> Divide and Conquer TSP Solution:");
